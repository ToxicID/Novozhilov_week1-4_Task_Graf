...
namespace Задание_3
{
    public partial class Form3 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            textBox1.Text += "Старый вариант" + Environment.NewLine;
            for (double i = a; i <= b; i += h)
            {
                textBox1.Text += $"f({i:f2})={f(i):f2}" + Environment.NewLine;
            }
            textBox1.Text += Environment.NewLine+"Перегруженный метод" + Environment.NewLine;
            for (double i = a; i <= b; i += h)
            {
                f(i, out double y);
                textBox1.Text += $"f({i:f2})={y:f2}" + Environment.NewLine;
            }
        }
    }
}
